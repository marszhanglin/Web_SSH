(function() {
    ...
    function addStartSugText() {
        ...
        var input = $("<input type='search' width='60px' id='startInput' value='省汽修' text='26,3111.42533851,-5527.73795456'></input>");
                ...
                if(!stopQueryEvent.result.transferStopInfos.length) {
                    ...
                    for(var i=0,len=stopQueryEvent.result.transferStopInfos.length; i<len; i++) {
                        ...
                        list.find("li").click(function() {
                            ...
                            function(){$(this).removeClass("dropListMouseOver");}    
                        );
                    } 
                }
            }
    ...
    function addEndSugText() {
        ...
        var input = $("<input type='search' width='60px' id='endInput' value='中安大厦' text='180,6055.3431955,-4270.0725196'></input>");
    ...
    function dowork() {       
        ...
        function getStartEndStopID() {
            if(!$("#startInput").attr("value")) {
                doMapAlert("查询提示", "请选择查询的起点");
                return;
            }
            if(!$("#endInput").attr("value")) {
                doMapAlert("查询提示", "请选择查询的终点");
                return;
            }

            var start = $("#startInput").attr("text");
            var startList = $("#startSugList li");
            if(startList.length === 1 && startList.html() !== "未找到匹配的起点") {
            ...
            if(endList.length === 1 && endList.html() !== "未找到匹配的终点") {
        ...
        function executeTrafficTransfer(points, transferTactic) {
            if(noStart) {
                doMapAlert("查询提示", "无匹配的起点", true);
                return;
            }
            if(noEnd) {
                doMapAlert("查询提示", "无匹配的终点", true);
                return;
            }
            window.paths = {};
    ...
    function execTransSolutionsQuery(points, tactic, preference, chked) {
        ...
        function transferSolutionsFailed(event) {
            doMapAlert("错误提示", event.error.errorMsg, true);
        }
    }
